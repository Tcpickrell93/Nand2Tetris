...
TESTS := ${shell find $(SRCTEST_DIR_DIR) -name '*.cpp' -or -name '*.c' -or -name '*.s'}
#$(info $$TESTS is [${TESTS}])
...
${BIN_DIR}/${EXECUTABLE} : ${TEST_OBJS} ${SRC_LIB} 
	g++ -o $@ ${TEST_OBJS} ${SRC_LIB} ${G++_LINKING_FLAGS}

${SRC_BUILD_DIR}/%.cpp.a : ${SRC_OBJS}
	ar rvs $@ $<

${SRC_BUILD_DIR}/%.cpp.o : ${SRC_DIR}/%.cpp ${SRC_INCS}
	mkdir -p $(dir $@)
